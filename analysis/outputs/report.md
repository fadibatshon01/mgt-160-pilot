...
### 3e. OLS regression with demographic controls

To check whether the null result above survives when we account for who took the survey, we ran a single OLS regression with:

- **Dummy variables for the experiment:** `hyped` (1 = hyped headline, 0 = neutral), `social_proof_yes` (1 = peer banner, 0 = none), and the interaction `hyped × social_proof_yes`.
- **Demographic controls:** `prior_investor` (yes/no), `age`, `gender` (baseline = man), `year_in_school` (baseline = freshman), `major_area` (baseline = business/econ). All categorical demographics entered as dummy variables.
- **Standard errors:** HC1 robust (per the course's spec).

**Treatment effects (the experiment), holding demographics constant:**

| effect | β ($) | 95% CI | p |
|---|---|---|---|
| Hyped headline (vs neutral) | −19.1 | [−154.0, +115.8] | 0.782 |
| Social-proof banner (vs none) | −35.4 | [−164.7, +93.9] | 0.592 |
| Hyped × Social proof (interaction) | +48.7 | [−146.4, +243.9] | 0.624 |

Model fit: R² = 0.061, adjusted R² = −0.020, overall F-test p < 0.001.

![](fig_regression_coefs.png)

**Plain-English takeaway.** Even after pulling out variation explained by demographics, neither cue's confidence interval excludes zero — same story as the t-tests in 3a/3b. The overall model F-test is significant only because of one demographic control (humanities majors allocated ≈ \$228 less to HLXE than the business/econ baseline; see `tab_regression.csv` for the full coefficient table). Treatment-effect estimates are essentially unchanged once demographics are added, which is the result you want to see — randomization worked, so controls were not load-bearing.
...
## 11. Single-slide pack — Regression

> Paste this section into a slide-writing prompt to get **one slide** on the regression.

**Slide title:** Treatment effects survive demographic controls — still null.

**One-line setup.** OLS regression of HLXE allocation ($, 0–1000) on treatment dummies + demographic controls, HC1 robust SEs.

**Dummy variables (the experiment):**
- `hyped` = 1 if headline = "Hot Pick", 0 if neutral
- `social_proof_yes` = 1 if peer-popularity banner shown, 0 if not
- `hyped × social_proof_yes` = interaction

**Demographic controls in the model:** prior investing experience, age, gender, year in school, major area (all categorical variables entered as dummies; baselines = man / freshman / business-econ).

**Treatment-effect results (the bullets for the slide):**
- Hyped headline: **β = −\$19**, 95% CI [−\$154, +\$116], p = 0.78
- Social-proof banner: **β = −\$35**, 95% CI [−\$165, +\$94], p = 0.59
- Interaction (Hyped × Social proof): **β = +\$49**, 95% CI [−\$146, +\$244], p = 0.62

**Model fit:** R² = 0.061, adj. R² = −0.020, F-test p < 0.001 (driven by one demographic, not the cues).

**Figure for the slide:** `outputs/fig_regression_coefs.png` — coefficient plot of the 3 treatment effects with 95% CIs (HC1).

**One-sentence interpretation (slide body or speaker note):** "After controlling for prior investing experience, age, gender, year in school, and major, neither cue moves HLXE allocation — every confidence interval still spans zero, confirming the null result from the t-tests is not a demographics artifact."

**Why this slide matters in the deck:** it's the robustness check for the headline null. Randomization should have already balanced demographics across cells (Section 2), and this regression confirms that — the treatment coefficients are essentially unchanged when controls are added, which is exactly what you want to see.
